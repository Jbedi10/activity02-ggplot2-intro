...
- `species`: Gentoo, Chinstrap, Adeleli
- `island`: where they are located
- `bill_length_mm`: length of bill
- `bill_depth_mm`: how far the bill goes down
- `flipper_length_mm`: length of flipper
- `body_mass_g`: weight
- `sex`: male or female
- `year`: when he or she was born
...
```{r boxplot-colors}
ggplot(data = penguins, mapping = aes(x = flipper_length_mm, y = species, colors = species)) +
  geom_boxplot()
```
...
**Response**:
Gentoo have the biggest flipper length, Chinstrap the second biggest and Adelie
...
**Response**:
boxplot is easier on the eyes as it gives a more clear definition of where the data lies. However, the jitterplot gives you an idea of the variations of flipper size for each penguin measured.
...
**Response**:
The MD files are alot easier to read as there is no code on the file!
...
**Response**:
geom_jitter() +geom_boxplot() set up is easier to read as the box plot is more visible.
...
**Response**:
i like having the points colored and the box plot black as it is more appealing to the eye and I am able to read the points easier.
